...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_pod"></a> [pod](#module\_pod) | registry.infrahouse.com/infrahouse/website-pod/aws | 4.4.0 |
...
| <a name="input_ecs_service_security_group_ids"></a> [ecs\_service\_security\_group\_ids](#input\_ecs\_service\_security\_group\_ids) | Security group ids which would be used if awsvpc network node enabled | `list(string)` | `[]` | no |
| <a name="input_ecs_service_subnets"></a> [ecs\_service\_subnets](#input\_ecs\_service\_subnets) | Subnets id's which would be used if awsvpc network node enabled | `list(string)` | `[]` | no |
...
| <a name="input_network_mode"></a> [network\_mode](#input\_network\_mode) | The network mode to use for the task. Default is bridge mode. | `string` | `"bridge"` | no |
